...
import type { ConversationTurn } from './persona-builder';
import { PreferenceInferer } from './preference-inferer';
...
export class ProfileManager {
  ...
  constructor(options?: ProfileManagerOptions) {
    ...
    this.config = {
      storage: {
        dbPath: options?.storagePath ?? './data/profile.db',
        enableCache: true,
        cacheSize,
      },
      persona: {
  ...
  private async getMemoriesByType(userId: string, type: MemoryType): Promise<Array<{ uid: string; content: string; updatedAt: number }>> {
    ...
    try {
      ...
      // 使用类型名称作为查询字符串以获得有效的向量搜索结果
      const result = await this.memoryService.recall({
        query: `[${type}]`,  // 使用类型标记作为查询
        types: [type],
        limit: 100,
      });
  ...
  private async savePersonaToMemory(persona: Persona): Promise<void> {
    ...
    try {
      ...
      this.logger.debug('Persona saved to MemoryService', { userId: persona.userId });
    } catch (error) {
  ...
  private async savePreferencesToMemory(userId: string, preferences: UserPreferences): Promise<void> {
    ...
    try {
      await this.memoryService.store(
        ...
        {
          importance: 7.0,   // PREFERENCE 高重要性
          scopeScore: 7.0,   // 全局有效
        }
      );
      this.logger.debug('Preferences saved to MemoryService', { userId });
    } catch (error) {
