...
import type { OMMSConfig, LogLevel, LLMProvider } from '../types/config';
...
export class ConfigValidator {
  ...
  private registerBuiltinRules(): void {
    ...
    // 文件路径验证
    this.registerRule('palace.basePath', {
      name: 'FilePath',
      validate: (value) => {
        const path = value as string;
        return typeof path === 'string' && path.length > 0;
      },
      message: 'palace.basePath must be a non-empty string',
      required: true,
    });
    
    // 正整数验证
    this.registerRule('vector.dimensions', {
      name: 'PositiveNumber',
      validate: (value) => {
        return typeof value === 'number' && value > 0 && Number.isInteger(value);
      },
      message: 'vector.dimensions must be a positive integer',
      required: true,
    });
    
    // LLM Provider 验证
    this.registerRule('llm.provider', {
      name: 'LLMProvider',
      validate: (value) => {
        const validProviders = ['minimax', 'openai', 'openai-compatible'];
        return validProviders.includes(value as string);
      },
      message: 'llm.provider must be one of: minimax, openai, openai-compatible',
      required: true,
    });
    
    // 评分范围验证 (0-1)
    this.registerRule('scoring.importance.confidenceWeight', {
      name: 'ScoreRange',
      validate: (value) => {
        return typeof value === 'number' && value >= 0 && value <= 1;
      },
      message: 'Score weights must be between 0 and 1',
      required: true,
    });
    
    // 数组验证
    this.registerRule('palace.wingTypes', {
      name: 'Array',
      validate: (value) => {
        return Array.isArray(value) && value.length > 0;
      },
      message: 'palace.wingTypes must be a non-empty array',
      required: true,
    });
    
    // 对象验证
    this.registerRule('palace', {
      name: 'Object',
      validate: (value) => {
        return typeof value === 'object' && value !== null && !Array.isArray(value);
      },
      message: 'palace must be an object',
      required: true,
    });
  }
